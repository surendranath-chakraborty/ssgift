 ...
 * Features:
 *  - Fully responsive (mobile bottom-tab nav)
 *  - Product image upload with canvas crop + resize editor
 *  - Full CRUD: products, orders, users, coupons
 */
const Admin = (() => {
  let _editId   = null;   // product being edited
  let _cropData = null;   // { img, scale, offsetX, offsetY, cropW, cropH, aspect, dragging, ... }
  let _croppedImageB64 = null; // final base64 result
  ...
  const switchSection = (sec) => {
    ...
    const map = { dashboard:renderDashboard, products:renderProducts,
                  orders:renderOrders, users:renderUsers, coupons:renderCoupons };
    if (map[sec]) map[sec]();
  };
  ...
  const renderDashboard = () => {
    ...
    if (tbody) tbody.innerHTML = orders.slice(0,5).map(o => `
      <tr>
  ...
  const renderProducts = () => {
    ...
    const prods = Store.getProducts().filter(p => !q || p.name.toLowerCase().includes(q) || p.cat.toLowerCase().includes(q));
    ...
    tbody.innerHTML = prods.map(p => `
      <tr>
        ...
        <td style="white-space:nowrap">
          <button class="action-btn action-btn--edit" onclick="Admin.openProductModal(${p.id})" title="Edit">
            <i class="fas fa-edit"></i>
          </button>
          <button class="action-btn action-btn--del" onclick="Admin.deleteProduct(${p.id})" title="Delete">
  ...
  const renderOrders = () => {
    ...
    tbody.innerHTML = orders.map(o => `
      <tr>
        ...
        <td>₹${o.total}</td>
        <td>
          <select class="form-input" style="padding:.3rem .6rem;font-size:var(--text-xs)"
      ...
      `<tr><td colspan="6" style="text-align:center;color:var(--color-muted);padding:2rem">No orders yet</td></tr>`;
  };
  ...
  const renderCoupons = () => {
    ...
    tbody.innerHTML = coupons.map((c,i) => `
      <tr>
        <td><strong style="letter-spacing:.05em;font-family:var(--font-mono,monospace)">${c.code}</strong></td>
    ...
    const currentImgHtml = p ? `
      <div class="img-current-preview" id="currentImgPreview">
        ${p.image
          ...
          : `<div class="emoji-preview">${p.emoji}</div>`}
        <div class="img-current-preview__info">
          <div class="img-current-preview__label">Current image</div>
          <div class="img-current-preview__name">${p.image ? 'Custom photo' : 'Emoji icon — ' + p.emoji}</div>
        </div>
        ${p.image ? `<button class="action-btn action-btn--del" onclick="Admin._removeCurrentImage()" title="Remove photo"><i class="fas fa-times"></i></button>` : ''}
      </div>` : '';
      ...
      <div style="display:flex;flex-direction:column;gap:var(--s4)">
        ...
        <div class="form-grid">
          <div class="form-group">
            <label>Product Name <span style="color:var(--color-rose)">*</span></label>
          ...
          <div class="form-group">
            <label>Category <span style="color:var(--color-rose)">*</span></label>
            <select class="form-input" id="mpCat">
              ${cats.map(c=>`<option value="${c}"${p&&p.cat===c?' selected':''}>${c}</option>`).join('')}
            </select>
          </div>
          <div class="form-group">
            <label>Price (₹) <span style="color:var(--color-rose)">*</span></label>
        ...
        <!-- Image Section -->
        <div>
          <label style="font-size:var(--text-sm);font-weight:600;color:var(--color-text-2);display:block;margin-bottom:var(--s2)">
            Product Image
          </label>
          ${currentImgHtml}
          <div class="img-upload-zone" id="imgUploadZone" ondragover="Admin._onDragOver(event)" ondragleave="Admin._onDragLeave(event)" ondrop="Admin._onDrop(event)">
            <input type="file" id="imgFileInput" accept="image/*" onchange="Admin._onFileSelect(event)"/>
            <div class="img-upload-zone__icon">📷</div>
            <div class="img-upload-zone__text">Click or drag a photo here</div>
            <div class="img-upload-zone__sub">JPG, PNG, WebP — max 5MB • You can crop &amp; resize after uploading</div>
          </div>

          <!-- Crop editor (hidden until image loaded) -->
          <div id="cropEditor" style="display:none;margin-top:var(--s4)">
            <div style="font-size:var(--text-sm);font-weight:600;color:var(--color-text-2);margin-bottom:var(--s3)">
              ✂️ Crop &amp; Resize
            </div>
            ...
            <div style="margin-bottom:var(--s3)">
              ...
              <div class="crop-ratio-btns">
                <button class="crop-ratio-btn active" data-ratio="1" onclick="Admin._setAspect(1,this)">1:1 Square</button>
                <button class="crop-ratio-btn" data-ratio="1.33" onclick="Admin._setAspect(1.33,this)">4:3</button>
                <button class="crop-ratio-btn" data-ratio="1.78" onclick="Admin._setAspect(1.78,this)">16:9</button>
                <button class="crop-ratio-btn" data-ratio="0.75" onclick="Admin._setAspect(0.75,this)">3:4 Portrait</button>
                <button class="crop-ratio-btn" data-ratio="0" onclick="Admin._setAspect(0,this)">Free</button>
              </div>
            </div>
            ...
            <div class="crop-controls">
              <div class="crop-control-group">
                ...
                <input type="range" class="crop-slider" id="zoomSlider"
                  min="0.1" max="3" step="0.01" value="1"
                  oninput="Admin._onZoom(this.value)"/>
              </div>
              <div class="crop-control-group">
                <label><i class="fas fa-sync-alt"></i> Rotate</label>
                <input type="range" class="crop-slider" id="rotateSlider"
                  min="-180" max="180" step="1" value="0"
                  oninput="Admin._onRotate(this.value)"/>
              </div>
            </div>

            <!-- Output size -->
            <div style="display:flex;gap:var(--s3);margin-top:var(--s3);flex-wrap:wrap">
              <div class="crop-control-group" style="flex:0 0 auto">
                <label>Output Width (px)</label>
                <input class="form-input" type="number" id="outWidth" value="600"
                  style="width:90px;padding:.4rem .6rem"/>
              </div>
              <div class="crop-control-group" style="flex:0 0 auto">
                <label>Output Height (px)</label>
                <input class="form-input" type="number" id="outHeight" value="600"
                  style="width:90px;padding:.4rem .6rem"/>
              </div>
            </div>

            <div class="crop-action-btns">
              <button class="btn btn--primary btn--md" onclick="Admin._applyCrop()">
                <i class="fas fa-crop-alt"></i> Apply Crop
              </button>
              <button class="btn btn--outline btn--md" onclick="Admin._resetCrop()">
                <i class="fas fa-undo"></i> Reset
              </button>
              <button class="btn btn--ghost btn--md" onclick="Admin._cancelCrop()">
            ...
            <div id="croppedPreview" style="display:none;margin-top:var(--s4)">
              <div style="font-size:var(--text-xs);color:var(--color-success);font-weight:600;margin-bottom:var(--s2)">
                ✅ Crop applied — this image will be saved with the product
              </div>
              <img id="croppedImg" style="max-width:160px;border-radius:var(--r-md);border:2px solid var(--color-success);display:block"/>
            </div>
          </div>
        </div>
  ...
  const _loadImageFile = (file) => {
    ...
    reader.onload = (ev) => {
      ...
      img.onload = () => {
        ...
        _setAspect(1, document.querySelector('.crop-ratio-btn.active'));
        _setupCanvas();
        _drawCrop();
      };
  ...
  const _setupCanvas = () => {
    ...
    canvas.width  = maxW;
    canvas.height = Math.min(maxW * ratio, 360);

    _cropData.panX     = 0;
    _cropData.panY     = 0;
    _cropData.scale    = 1;
    _cropData.rotation = 0;

    // Default crop box = centered square (or aspect)
    const asp = _cropData.aspect || 1;
    const cW  = Math.min(canvas.width  * 0.8, canvas.height * asp * 0.8);
    const cH  = asp === 0 ? cW : cW / asp;
    _cropData.cropX = (canvas.width  - cW) / 2;
    _cropData.cropY = (canvas.height - cH) / 2;
    _cropData.cropW = cW;
    _cropData.cropH = cH;
  ...
  const _hitHandle = (px,py) => {
    ...
    const hs = 10;
    const handles = {
      'tl':{hx:x,    hy:y    }, 'tr':{hx:x+w,  hy:y    },
      'bl':{hx:x,    hy:y+h  }, 'br':{hx:x+w,  hy:y+h  },
      'tm':{hx:x+w/2,hy:y    }, 'bm':{hx:x+w/2,hy:y+h  },
      'ml':{hx:x,    hy:y+h/2}, 'mr':{hx:x+w,  hy:y+h/2},
    };
  ...
  const _onCanvasDown = (e) => {
    ...
    const c  = document.getElementById('cropCanvas');
    const p  = _getPos(e, c);
    const h  = _hitHandle(p.x, p.y);
    if (h) {
      _cropData.resizing       = true;
      _cropData.dragging       = false;
      _cropData.resizeHandle   = h;
      _cropData.resizeStartX   = p.x;
      _cropData.resizeStartY   = p.y;
      _cropData.resizeCropStart= { x:_cropData.cropX,y:_cropData.cropY,w:_cropData.cropW,h:_cropData.cropH };
    } else if (_inCropBox(p.x,p.y)) {
      _cropData.dragging = true;
      _cropData.resizing = false;
      _cropData.lastX = p.x; _cropData.lastY = p.y;
    } else {
      // Pan image
      _cropData.dragging  = false;
      _cropData.resizing  = false;
      _cropData._panning  = true;
      _cropData.lastX = p.x; _cropData.lastY = p.y;
    }
  };
  ...
  const _onCanvasMove = (e) => {
    const c = document.getElementById('cropCanvas');
    if (!c || !_cropData) return;
    const p  = _getPos(e, c);

    if (_cropData.resizing) {
      const dx = p.x - _cropData.resizeStartX;
      const dy = p.y - _cropData.resizeStartY;
      const s  = _cropData.resizeCropStart;
      const asp= _cropData.aspect;
      let { x,y,w,h } = { x:s.x, y:s.y, w:s.w, h:s.h };
      const hr = _cropData.resizeHandle;
      ...
      if (asp && asp!==0) { h = w / asp; }  // lock aspect

      _cropData.cropX = x; _cropData.cropY = y;
      _cropData.cropW = w; _cropData.cropH = h;
      _drawCrop();
      return;
    }

    if (_cropData.dragging) {
      _cropData.cropX += p.x - _cropData.lastX;
      _cropData.cropY += p.y - _cropData.lastY;
      _cropData.lastX  = p.x; _cropData.lastY = p.y;
      _drawCrop();
      return;
    }

    if (_cropData._panning) {
      _cropData.panX += p.x - _cropData.lastX;
      _cropData.panY += p.y - _cropData.lastY;
      _cropData.lastX = p.x; _cropData.lastY = p.y;
      _drawCrop();
    }
    ...
    c.style.cursor = h ? (cursors[h]||'crosshair') : _inCropBox(p.x,p.y) ? 'move' : 'crosshair';
  };
  ...
  const _drawCrop = () => {
    const canvas = document.getElementById('cropCanvas');
    if (!canvas || !_cropData?.img) return;
    ...
    // Draw image with pan/zoom/rotation
    ...
    handles.forEach(([hx,hy]) => {
      ctx.fillStyle   = '#fff';
      ctx.strokeStyle = 'var(--color-rose,#b5606a)';
      ctx.lineWidth   = 1.5;
      ctx.beginPath();
    ...
    ctx.fillRect(cropX, cropY-22, 90, 20);
    ctx.fillStyle = '#fff';
  ...
  const _setAspect = (ratio, btn) => {
    ...
    if (_cropData.aspect && _cropData.cropW) {
      _cropData.cropH = _cropData.cropW / _cropData.aspect;
    }
    _drawCrop();
  };
  ...
  const _applyCrop = () => {
    ...
    ctx.save();
    // Scale from crop-box coords to output coords
    ctx.scale(outW/cropW, outH/cropH);
    ctx.translate(-cropX, -cropY);
    // Draw image with same transform as preview
    const imgAspect = img.width / img.height;
    const fitH = H;
    const fitW = fitH * imgAspect;
    const drawW = fitW * scale;
    ...
    // Show preview
    const prev = document.getElementById('croppedPreview');
    const img2 = document.getElementById('croppedImg');
    if (prev && img2) {
      img2.src = _croppedImageB64;
      prev.style.display = 'block';
    }
    Toast.show('Crop applied! Save product to confirm.', 'success');
  };
  ...
  const _resetCrop = () => {
    ...
    _drawCrop();
    _croppedImageB64 = null;
    if(document.getElementById('croppedPreview'))
      document.getElementById('croppedPreview').style.display='none';
  };
  ...
  const _removeCurrentImage = () => {
    ...
    if (prev) prev.remove();
    // Signal to remove image on save
  ...
  const saveProduct = () => {
    ...
    // Handle image
    if (_croppedImageB64 === '__remove__') {
      base.image = null;
    } else if (_croppedImageB64) {
      base.image = _croppedImageB64;
    }
    ...
    if (_editId) {
      ...
      if (idx >= 0) {
        products[idx] = {
          ...products[idx], ...base,
          // Only override image if explicitly changed
          ...(_croppedImageB64 !== null ? { image: _croppedImageB64 === '__remove__' ? null : _croppedImageB64 } : {}),
        };
      }
    } else {
  ...
  const openCouponModal = () => {
    Modal.open('Add New Coupon', `
      <div class="form-grid">
        <div class="form-group">
          <label>Coupon Code <span style="color:var(--color-rose)">*</span></label>
        ...
        <div class="form-group form-group--full">
          <label>Value <span style="color:var(--color-rose)">*</span></label>
  ...
  return {
    ...
    _onZoom, _onRotate, _setAspect,
    _applyCrop, _resetCrop, _cancelCrop, _removeCurrentImage,
  };
})();