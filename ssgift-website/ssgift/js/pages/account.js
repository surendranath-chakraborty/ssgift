        ...
        <div class="auth-form active" id="loginForm">
          ...
          <div class="form-group">
            ...
            <div class="password-wrap">
              <input class="form-input" type="password" id="loginPass"
                placeholder="••••••••" autocomplete="current-password"
                onkeydown="if(event.key==='Enter') Auth.login()"/>
              <button class="password-toggle" type="button"
                onclick="Auth.togglePassword('loginPass', this)" tabindex="-1">
                <i class="fas fa-eye"></i>
              </button>
            </div>
          </div>

          <div style="text-align:right;margin-top:calc(var(--s1) * -1)">
            <button class="forgot-link" onclick="Auth.forgotPassword()">
              Forgot password?
            </button>
          </div>
        ...
        <div class="auth-form" id="registerForm">
          ...
          <div class="form-group">
            ...
            <input class="form-input" type="tel" id="regPhone"
              placeholder="+91 XXXXXXXXXX" autocomplete="tel"/>
          </div>
          <div class="form-group">
            <label>Password <span style="color:var(--color-rose)">*</span></label>
            <div class="password-wrap">
              <input class="form-input" type="password" id="regPass"
                placeholder="Min. 6 characters" autocomplete="new-password"
                onkeydown="if(event.key==='Enter') Auth.register()"/>
              <button class="password-toggle" type="button"
                onclick="Auth.togglePassword('regPass', this)" tabindex="-1">
                <i class="fas fa-eye"></i>
              </button>
            </div>
          </div>
...
const AccountPage = (() => {
  const render = () => {
    ...
    const avatarInner = u.photo
      ? `<img src="${u.photo}" alt="${u.name}" onerror="this.style.display='none'">`
        ...
        <div class="account-content">
          <div class="account-section active" id="acc-orders">
            ...
            ${orders.length ? orders.map(o => `
              <div class="order-card">
                <div class="order-card__header">
                  ...
                  <span class="badge-status badge-${(o.status||'processing').toLowerCase()}">${o.status}</span>
                </div>
            ...
            : `<div class="empty-state">
                ...
                <p class="empty-state__desc">Your orders will appear here</p>
                <button class="btn btn--primary btn--md" onclick="Router.go('shop')"><i class="fas fa-store"></i> Shop Now</button>
               </div>`}
          </div>
          ...
          <div class="account-section" id="acc-profile">
            ...
            <div class="form-grid">
              ...
              <div class="form-group">
                <label>Email Address</label>
                <input class="form-input" type="email" id="profEmail" value="${u.email||''}" readonly title="Email cannot be changed"/>
              </div>
              <div class="form-group">
                <label>Phone Number</label>
                <input class="form-input" type="tel" id="profPhone" value="${u.phone||''}" placeholder="+91 XXXXXXXXXX"/>
              </div>
            </div>
            <button class="btn btn--primary btn--md" style="margin-top:var(--s6)" onclick="AccountPage.saveProfile()">
  ...
  const render = () => `
    <div class="admin-wrap">
      ...
      <aside class="admin-sidebar">
        <div class="admin-sidebar__brand"><span style="font-size:1.1rem">⚙️</span> Admin Panel</div>
        <ul class="admin-menu">
      ...
      <nav class="admin-tab-nav">
        <div class="admin-tab-nav__items">
          <button class="admin-tab-nav__item active" data-sec="dashboard" onclick="Admin.switchSection('dashboard')"><i class="fas fa-chart-bar"></i> Stats</button>
          <button class="admin-tab-nav__item" data-sec="products"  onclick="Admin.switchSection('products')"><i class="fas fa-gift"></i> Products</button>
          <button class="admin-tab-nav__item" data-sec="orders"    onclick="Admin.switchSection('orders')"><i class="fas fa-box"></i> Orders</button>
          <button class="admin-tab-nav__item" data-sec="users"     onclick="Admin.switchSection('users')"><i class="fas fa-users"></i> Users</button>
          <button class="admin-tab-nav__item" data-sec="coupons"   onclick="Admin.switchSection('coupons')"><i class="fas fa-tag"></i> Coupons</button>
        </div>
      </nav>
